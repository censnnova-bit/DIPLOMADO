...
const cargarReservasOcupadas = async () => {
  try {
    const response = await api.getReservas({ estado: 'aprobada' })
...
<template>
  ...
  <div v-if="showNotificaciones && reservasOcupadas.length > 0" class="fixed top-20 right-4 z-50 w-96 bg-white dark:bg-gray-800 shadow-2xl rounded-lg border-l-4 border-[#B90A0A] animate-slide-in">
    <div class="p-4">
      ...
      <router-link to="/admin/reservas" @click="cerrarNotificaciones" class="block mt-3 text-center text-sm text-[#B90A0A] font-semibold hover:underline">
