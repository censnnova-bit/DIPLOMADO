...
const handleFileUpload = (event) => {
  salonForm.value.imagen_file = event.target.files[0]
}
...
const loadSalones = async () => {
  ...
  try {
    ...
    notification.error('Error al cargar los salones')
  } finally {
    loading.value = false
  }
}
...
const saveSalon = async () => {
  try {
    ...
    Object.keys(salonForm.value).forEach(key => {
      // Manejo especial para la imagen
      if (key === 'imagen_file') {
        if (salonForm.value.imagen_file) {
          formData.append('imagen', salonForm.value.imagen_file)
          // Si subimos imagen nueva, limpiamos la URL anterior por seguridad
          formData.append('imagen_url', '')
        }
      } else {
        const value = salonForm.value[key]
    ...
    if (editingId.value) {
      await api.updateSalon(editingId.value, formData)
      notification.success('Salón actualizado exitosamente')
    } else {
      await api.createSalon(formData)
      notification.success('Salón creado exitosamente')
    }
    showModal.value = false
    loadSalones()
  } catch (err) {
    console.error('Error guardando salón:', err)
    let msg = 'Error al guardar el salón'
...
const deleteSalon = async (id, nombre) => {
  ...
  try {
    ...
    notification.success('Salón eliminado exitosamente')
    loadSalones()
  } catch (err) {
    console.error('Error eliminando salón:', err)
    notification.error('Error al eliminar el salón')
  }
}
</script>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="salon in salones" :key="salon.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">{{ salon.codigo
              }}</td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ salon.capacidad }}
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm">
                <div class="flex justify-end space-x-2">
                  <button @click="openEditModal(salon)"
                  ...
                  <button @click="deleteSalon(salon.id, salon.nombre)"
            ...
            <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input v-model="salonForm.nombre" required type="text"
                ...
                <div>
                  ...
                  <input v-model="salonForm.codigo" required type="text"
                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-[#B90A0A] focus:ring-[#B90A0A]">
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Tipo *</label>
                  <select v-model="salonForm.tipo" required
                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-[#B90A0A] focus:ring-[#B90A0A]">
                ...
                <div>
                  ...
                  <select v-model="salonForm.estado" required
                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-[#B90A0A] focus:ring-[#B90A0A]">
                ...
                <div>
                  ...
                  <input v-model="salonForm.bloque" required type="text"
                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-[#B90A0A] focus:ring-[#B90A0A]">
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Piso *</label>
                  <input v-model="salonForm.piso" required type="text"
                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-[#B90A0A] focus:ring-[#B90A0A]">
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Capacidad *</label>
                  <input v-model.number="salonForm.capacidad" required type="number" min="1"
                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-[#B90A0A] focus:ring-[#B90A0A]">
                </div>

                <div class="col-span-2">
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Descripción</label>
                  <textarea v-model="salonForm.descripcion" rows="3"
                ...
                <div class="col-span-2">
                  ...
                  <input @change="handleFileUpload" type="file" accept="image/*"
                    class="mt-1 block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-[#B90A0A] file:text-white hover:file:bg-[#8f0606]">
                </div>

                <div class="col-span-2">
                  <span class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Características</span>
                  <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    <label class="flex items-center">
                      <input v-model="salonForm.tiene_proyector" type="checkbox"
                        class="rounded text-[#B90A0A] focus:ring-[#B90A0A]">
                      <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Proyector</span>
                    </label>
                    <label class="flex items-center">
                      <input v-model="salonForm.tiene_aire_acondicionado" type="checkbox"
                        class="rounded text-[#B90A0A] focus:ring-[#B90A0A]">
                      <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Aire Acondicionado</span>
                    </label>
                    <label class="flex items-center">
                      <input v-model="salonForm.tiene_computadores" type="checkbox"
                        class="rounded text-[#B90A0A] focus:ring-[#B90A0A]">
                      <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Computadores</span>
                    </label>
                    <label class="flex items-center">
                      <input v-model="salonForm.tiene_smart_tv" type="checkbox"
                        class="rounded text-[#B90A0A] focus:ring-[#B90A0A]">
                      <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Smart TV</span>
                    </label>
                    <label class="flex items-center">
                      <input v-model="salonForm.tiene_audio" type="checkbox"
                        class="rounded text-[#B90A0A] focus:ring-[#B90A0A]">
                      <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Sistema de Audio</span>
                    </label>
                    <label class="flex items-center">
                      <input v-model="salonForm.tiene_wifi" type="checkbox"
                        class="rounded text-[#B90A0A] focus:ring-[#B90A0A]">
